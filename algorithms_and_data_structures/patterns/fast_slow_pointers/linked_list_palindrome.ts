import { ILinkedListNode } from '../../linked_list/model';
import { getListItemsString, log } from '../../linked_list/utils';

// https://www.educative.io/courses/grokking-the-coding-interview/JERG3q0p912
function isPalindrome(head: ILinkedListNode): boolean {
  let slow = head,
    fast = head;
  while (fast?.next) {
    slow = slow.next;
    fast = fast.next.next;
  }

  let secondHalfHead = reverse(slow);
  let copySecondHalfHead = secondHalfHead;
  while (head && secondHalfHead) {
    log('head', head.val, 'secondHalfHead', secondHalfHead.val);
    if (head.val !== secondHalfHead.val) return false;
    head = head.next;
    secondHalfHead = secondHalfHead.next;
  }
  reverse(copySecondHalfHead);

  return true;
}

function reverse(head: ILinkedListNode): ILinkedListNode {
  let prev = null,
    next = null;

  while (head) {
    next = head.next;
    head.next = prev;
    prev = head;
    head = next;
  }

  return prev;
}

export function testPalindrome() {
  const head: ILinkedListNode = { val: 1, next: null };
  head.next = { val: 2, next: null };
  head.next.next = { val: 3, next: null };
  head.next.next.next = { val: 3, next: null };
  head.next.next.next.next = { val: 2, next: null };
  head.next.next.next.next.next = { val: 1, next: null };
  log('Before', getListItemsString(head));
  log('Expected true, got: ', isPalindrome(head));
  log('After', getListItemsString(head));

  const head2: ILinkedListNode = { val: 1, next: null };
  head2.next = { val: 2, next: null };
  head2.next.next = { val: 3, next: null };
  head2.next.next.next = { val: 2, next: null };
  head2.next.next.next.next = { val: 1, next: null };
  log('Before', getListItemsString(head2));
  log('Expected true, got: ', isPalindrome(head2));
  log('After', getListItemsString(head2));

  head2.next.next.next.next.next = { val: 1, next: null };
  log('Before', getListItemsString(head2));
  log('Expected false, got: ', isPalindrome(head2));
  log('After', getListItemsString(head2));
}
